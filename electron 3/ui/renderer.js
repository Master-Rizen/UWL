...
async function loadWorkspaces() {
    ...
    Object.keys(data).forEach(name => {
        if (name === "_lastWorkspace") return; // ignore config key

        const div = document.createElement("div");
        div.textContent = name;
        div.className = "workspace-item";
        div.dataset.name = name; // 🔥 store clean reference
...
async function selectWorkspace(name) {
    ...
    await window.api.setLastWorkspace(name);
    ...
    <div class="workspace-stats">
        <div>
            <h4>${Math.floor(ws.timeLogged / 60)} mins</h4>
            <p>Total Time</p>
        </div>

        <div>
            <h4>${ws.blockedApps?.length || 0}</h4>
            <p>Blocked Apps</p>
        </div>
    </div>
`;
...
focusBtn.addEventListener("click", async () => {
    ...
    if (!focusActive) {
        ...
        focusBtn.classList.remove("primary-btn");
        focusBtn.classList.add("danger-btn");

        document.getElementById("focusStatus").textContent = "Active";
        document.getElementById("focusStatus").style.color = "#22c55e";
        focusActive = true;
        ...
        focusBtn.classList.remove("danger-btn");
        focusBtn.classList.add("primary-btn");

        document.getElementById("focusStatus").textContent = "Inactive";
        document.getElementById("focusStatus").style.color = "#ef4444";
        focusActive = false;
    }
});
...
async function initApp() {
    ...
    const last = await window.api.getLastWorkspace();
